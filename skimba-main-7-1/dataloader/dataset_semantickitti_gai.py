...
class voxel_dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if len(data) == 7:
            feature_GT, feature_VAE, feature_partial, feature_image, origin_len, dir_idx, number_idx= data
        else:
            raise Exception('Return invalid data tuple')

        data_tuple = (feature_GT, feature_VAE, feature_partial, feature_image, origin_len, dir_idx, number_idx)
...
class cylinder_dataset(data.Dataset):
    ...
    def get_single_sample(self, data, index, vote_idx=0):
        if len(data) == 2:
            ...
            xyz, labels, sig, origin_len,labels_points = data
            if len(sig.shape) == 2: sig = np.squeeze(sig)
        else:
            raise Exception('Return invalid data tuple')
        ...
        grid_ind = (np.floor((np.clip(xyz_pol, min_bound, max_bound) - min_bound) / intervals)).astype(np.int_)
        ...
        elif len(data) == 3 or len(data) == 4 or len(data) == 5:
...
def collate_fn_BEV(data):
    data2stack_GT = np.stack([d[0] for d in data]).astype(np.int_)
    data2stack_VAE = np.stack([d[1] for d in data]).astype(np.float32)
    data2stack_partial = np.stack([d[2] for d in data]).astype(np.float32)
    data2stack_image = np.stack([d[3] for d in data]).astype(np.float32)
    grid_ind_stack = [d[4] for d in data]
    point_label = [d[5] for d in data]
    xyz = [d[6] for d in data]
    return torch.from_numpy(data2stack_GT), torch.from_numpy(data2stack_VAE), torch.from_numpy(data2stack_partial), torch.from_numpy(data2stack_image), grid_ind_stack, point_label, xyz
...
def collate_fn_BEV_ms_tta(data):
    ...
    label2stack = np.stack([d[1] for d in data]).astype(np.int_)
    ...
    return torch.from_numpy(data2stack), torch.from_numpy(
        label2stack), grid_ind_stack, point_label, xyz, index  # , origin_len