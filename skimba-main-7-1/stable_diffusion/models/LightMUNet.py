...
class MambaLayer(nn.Module):
    def __init__(self, input_dim, output_dim, d_state=16, d_conv=4, expand=2):
        ...
        ## xy plane
        self.norm_xy = nn.LayerNorm(input_dim)
        self.mamba_xy = Mamba(
        ...
        self.proj_xy = nn.Linear(input_dim, output_dim)
        self.skip_scale_xy = nn.Parameter(torch.ones(1))
        ## xz plane
        self.norm_xz = nn.LayerNorm(input_dim)
        self.mamba_xz = Mamba(
            d_model=input_dim,  # Model dimension d_model
            d_state=d_state,  # SSM state expansion factor
            d_conv=d_conv,  # Local convolution width
        ...
        self.proj_xz = nn.Linear(input_dim, output_dim)
        self.skip_scale_xz = nn.Parameter(torch.ones(1))
        ## yz plane
        self.norm_yz = nn.LayerNorm(input_dim)
        self.mamba_yz = Mamba(
            d_model=input_dim,  # Model dimension d_model
            d_state=d_state,  # SSM state expansion factor
            d_conv=d_conv,  # Local convolution width
        ...
        self.proj_yz = nn.Linear(input_dim, output_dim)
        self.skip_scale_yz = nn.Parameter(torch.ones(1))
    ...
    def forward(self, x):
        ...
        ## xy plane
        x_xy = x.permute(0, 4, 3, 2, 1).flatten(start_dim=3).permute(0, 2, 1)
        x_norm_xy = self.norm_xy(x_xy)
        x_mamba_xy = self.mamba_xy(x_norm_xy) + self.skip_scale_xy * x_xy
        x_mamba_xy = self.norm_xy(x_mamba_xy)
        x_mamba_xy = self.proj_xy(x_mamba_xy)
        out_xy = x_mamba_xy.transpose(-1, -2).reshape(B, self.output_dim, *img_dims)

        ## xz plane
        x_xz = x.permute(0, 4, 2, 3, 1).flatten(start_dim=3).permute(0, 2, 1)
        x_norm_xz = self.norm_xz(x_xz)
        x_mamba_xz = self.mamba_xz(x_norm_xz) + self.skip_scale_xz * x_xz
        x_mamba_xz = self.norm_xz(x_mamba_xz)
        x_mamba_xz = self.proj_xz(x_mamba_xz)
        out_xz = x_mamba_xz.transpose(-1, -2).reshape(B, self.output_dim, *img_dims)

        ## yz plane
        x_yz = x.permute(0, 3, 4, 2, 1).flatten(start_dim=3).permute(0, 2, 1)
        x_norm_yz = self.norm_yz(x_yz)
        x_mamba_yz = self.mamba_yz(x_norm_yz) + self.skip_scale_yz * x_yz
        x_mamba_yz = self.norm_yz(x_mamba_yz)
        x_mamba_yz = self.proj_yz(x_mamba_yz)
        out_yz = x_mamba_yz.transpose(-1, -2).reshape(B, self.output_dim, *img_dims)

        out_all = out_xy + out_xz + out_yz
