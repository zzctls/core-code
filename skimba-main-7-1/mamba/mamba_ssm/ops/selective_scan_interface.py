...
def causal_conv1d_fwd_compat(x, weight, bias, silu_activation):
    if causal_conv1d_fwd_function is not None:
        return causal_conv1d_fwd_function(
            x, weight, bias, None, None, None, silu_activation
        )
    return causal_conv1d_cuda.causal_conv1d_fwd(
        x, weight, bias, silu_activation
    )


def causal_conv1d_bwd_compat(x, weight, bias, dout, dx, silu_activation):
    if causal_conv1d_bwd_function is not None:
        dx, dweight, dbias, *_ = causal_conv1d_bwd_function(
            x, weight, bias, dout, None, None, None, dx, False, silu_activation
        )
        return dx, dweight, dbias
    return causal_conv1d_cuda.causal_conv1d_bwd(
        x, weight, bias, dout, dx, silu_activation
    )
...
class MambaInnerFnNoOutProj(torch.autograd.Function):
    ...
    def forward(ctx, xz, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight,
        ...
        conv1d_out = causal_conv1d_fwd_compat(x, conv1d_weight, conv1d_bias, True)
    ...
    def backward(ctx, dout):
        ...
        dx, dconv1d_weight, dconv1d_bias = causal_conv1d_bwd_compat(
